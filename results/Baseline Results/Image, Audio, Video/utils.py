...
def write_image_summary(image_resolution, model, model_input, gt,
    ...
    img_laplace = model_output['model_out'].detach()  # skipped to save VRAM
...
#for RBF Model
def write_image_summary(image_resolution, ground_truth_img, model, model_input, gt,
                        model_output, writer, total_steps, prefix='train_'):
    ...
    with torch.no_grad():
        ...
        writer.add_image(prefix + 'gt_vs_pred', make_grid(output_vs_gt, scale_each=False, normalize=True),
                         global_step=total_steps)
        torch.cuda.empty_cache()
    write_psnr(dataio.lin2img(model_output['model_out'].detach(), image_resolution),
               dataio.lin2img(gt['img'].detach(), image_resolution), writer, total_steps, prefix+'img_')
